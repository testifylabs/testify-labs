...
export class HomePage {
  ...
  constructor(page: Page) {
    ...
    // Hero
    this.heroHeading = page.getByText('Enhance Your Software Product', { exact: false });
    this.heroSubtext1 = page.getByText(/Elevate your SDLC with our proprietary testing framework/i);
    this.heroSubtext2 = page.getByText(/Software Testing.*Quality Assurance.*on-Demand/i);
    ...
    // Features
    this.featureOnDemandHeading = page.getByText('On-Demand Software Quality Assurance', { exact: false });
    this.featureSpeedText = page.getByText('Speed', { exact: true });
    this.featureSpeedHeading = page.getByRole('heading', { name: 'Same Day Turnaround Time' });
    this.featureFlexibilityText = page.getByText('Flexibility', { exact: true });
    this.featureFlexibilityHeading = page.getByRole('heading', { name: /Quality Delivered on Your Schedule/i });
    this.featureReliabilityText = page.getByText('Reliability', { exact: true });
    ...
    // Marketplace CTA
    this.marketplaceCtaHeading = page.getByRole('heading', { name: /Try On-Demand quality assurance/i });
  ...
  async goto() {
    await this.page.goto('https://testifysolutions.net/', { waitUntil: 'domcontentloaded' });
  }
}
